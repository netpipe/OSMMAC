#include "cashDetector.h"


void CashDetector::loadFiles(cv::String faceCascadeFilename,
...
CashDetector::~CashDetector()
{

}

void CashDetector::processFrame(const cv::Mat &frame)
{
    if (processAll_)
        process(frame);
    else
...
void CashDetector::setProcessAll(bool all)
{
    processAll_ = all;
}

void CashDetector::process(cv::Mat frame)
{
    cv::Mat grey_image;
...
void CashDetector::timerEvent(QTimerEvent *ev)
{
...
void CashDetector::queue(const cv::Mat &frame)
{
    if (!frame.empty())
...
void CashDetector::matDeleter(void *mat)
{
    delete static_cast<cv::Mat*>(mat);
}

void CashDetector::facecascade_filename(QString filename)
{
